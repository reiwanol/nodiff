"""
Model definition for NoDiffTransformer (cleaned).
"""
...
from diff_transformer_utils import ImageProjection, RelativePositionAttention, ModifiedMLP, interpolate, convs_no_relu, convs, AttentionDecoderBlock
...
class ExchangeBlock(nn.Module):
    ...
    def forward(self, x1, x2):
        b, c, h, w = x1.shape
        exchange_mask = (torch.arange(c) % self.p_swap) == 0
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels=[64, 128, 256, 512],
        ...
        self.conv_outs = []
        for i in range(n):
            self.attn_decoders.append(AttentionDecoderBlock(hs[i], ws[i], in_channels[i], pos_dim=pos_dim))
    ...
    def forward(self, x_1, x_2):
        ...
        for i in range(len(x_1)):
            out = self.attn_decoders[i](x_1[i], x_2[i]).permute(0,2,1).reshape(n, -1, self.hs[i], self.ws[i])
...
class EncoderTransformer(nn.Module):
    ...
    def forward(self, src, temp):
        ...
        for i in range(self.n):
            ...
            x1, x2 = self.exchanges[i](x1, x2)
            src_outs.append(x1)
            temp_outs.append(x2)

        return src_outs, temp_outs
...
class NoDiffTransformer(nn.Module):
    ...
    def forward(self, src, temp):
        fx1, fx2 = self.enc(src, temp)
        diff = self.dec(fx1, fx2)
        return diff

__all__ = ['NoDiffTransformer']
